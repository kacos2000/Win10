...
Try {
	...
	Write-host "Total events with ID: 4616 = " -f Yellow -nonewline; Write-Host $Event.count -f White
...
$Events = foreach ($i in $xmllog) {
			...
			EventRecordID = $i.Event.System.EventRecordID
			ProcessID = $i.Event.System.Execution.ProcessID
