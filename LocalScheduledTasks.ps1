<#
	.SYNOPSIS
		Gets the Scheduled Tasks from TaskCache
	...
	.EXAMPLE
		PS C:\> .\LocalScheduledTasks.ps1
	
	.NOTES
		Costas Katsavounidis © 2023
...
if(!([Security.Principal.WindowsPrincipal][Security.Principal.WindowsIdentity]::GetCurrent()).IsInRole([Security.Principal.WindowsBuiltInRole] "Administrator"))
{
	...
	Write-Host "Script Requires Admin Rights`nPlease restart PowerShell As Admin and try running the script again." -ForegroundColor Red 
	Exit
}
...
$Tasks = foreach($Task in $registrySubKeyNames){
